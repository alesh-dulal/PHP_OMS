...
*/
    ...
    public function actionResetpwd($userID, $Email)
    {
        if($userID != " " && $Email != " "){  
            $resetLink =  self::GetPasswordResetLinkLink($userID);
    ...
    public function GetPasswordResetLinkLink($userID)
    {
        $passwordKey=Yii::$app->crypto->RandomKey(112);
        $connection = Yii::$app->getDb();
        $command = $connection->createCommand("update user set IsPasswordReset='1', PasswordKey ='".$passwordKey."' WHERE userid='".$userID."';
");
        $command->execute();
