...
class PayandterminateController extends \yii\web\Controller
{
    ...
    public function actionEmployeeabsentdays($id = NULL)
    {
        ...
        $lastDateOfPrevMonth = date("Y-m-d");
        ...
        $connection = Yii::$app->getDb();
        $Employee = $connection->createCommand("SELECT `EmployeeID`, `FullName`,`BankAccountNumber`,`Email`, `Salary` from `employee` WHERE EmployeeID NOT IN (SELECT MIN(EmployeeID) FROM employee) AND IsActive = 1 AND EmployeeID=".$id."");
    ...
    {
        $connection = Yii::$app->getDb();
        $Employee = $connection->createCommand("SELECT `FullName` from `employee` WHERE EmployeeID = ".$id);
