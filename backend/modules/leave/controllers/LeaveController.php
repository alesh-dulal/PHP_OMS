    ...
    public function actionIndex()
    {
        ...
        if($EmployeeLeaveModel->load(Yii::$app->request->post())) {
            ...
            if ($EmployeeLeaveModel['NoOfDays'] <= $AvailableDays['Balance']) {
                ...
                Yii::$app->session->setFlash('leaverequestedcancle', "Leave Requested Cancelled.<br/> Not Enough Balance. ");
                return $this->redirect('index');
            }
        } else{ 
            ...
            $query1 = new Query();
            $Result = $query1->select(['L.LeaveTypeID','LT.Title','L.Balance', 'L.Earned'])->where(['L.EmployeeID'=>Yii::$app->session['UserID']])->from('leave L')->leftJoin('listitems LT','LT.ListItemID = L.LeaveTypeID')->all();
